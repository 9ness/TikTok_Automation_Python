      ...
      <CardContent className="space-y-3 p-4 sm:p-5">
        ...
        {expanded && (
          <div className="space-y-3 border-t pt-3">
            {/* Tier + estrategia */}
            <CollapsibleCard title="Tier & estrategia" defaultOpen>
                ...
                <div className="grid gap-2 sm:grid-cols-3">
                  {ATLAS_TIERS.map((t) => (
                    <button
                      ...
                      onClick={() => {
                        patch("tier", t.value);
                        const opts: string[] = RES_BY_TIER[t.value] ?? ["720p"];
                        if (!opts.includes(config.resolution)) {
                          patch("resolution", opts[0] ?? "720p");
                        }
                      }}
