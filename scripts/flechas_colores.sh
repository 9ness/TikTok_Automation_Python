...
echo "$COLORES" | while read -r nombre r g b; do
    [[ -z "$nombre" ]] && continue
    destino="$DIR/flecha_${nombre}.mov"
    tmp="$(mktemp -d)/flecha_${nombre}.mov"
    # `-nostdin`: sin él, ffmpeg se come la lista de colores que le llega al
    # bucle por la tubería y los nombres salen cortados ("flecha_rron").
