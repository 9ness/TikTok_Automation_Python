"""Wrapper Upstash REST especializado para Cuenta Piloto (`cuenta_piloto:`).
...
from src.cuenta_piloto.config import REDIS_PREFIX


class CuentaPilotoRedis:
    """Cliente Redis con prefijo `cuenta_piloto:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[CuentaPilotoRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[CuentaPilotoRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: CuentaPilotoRedis | None = None


def get_cuenta_piloto_redis() -> CuentaPilotoRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = CuentaPilotoRedis()
    return _INSTANCE
