...
Key: `nicho_bof_cine:folder:<source>:<carpeta>`
"""
...
CAMPOS_PRIVADOS = frozenset({
    ...
    # Los DOS clips que sube el operador antes de montar. Van aquí, con lo
    # privado, porque el material subido es de quien lo sube.
...
def _require_redis():
    ...
    if not r.is_available():
        raise RuntimeError(
            ...
            "estado del Nicho BOF Cinematográfico."
        )
    return r
...
def load_folder_para(source: str, folder: str, usuario: str = "") -> dict:
    ...
    if _es_compartido(usuario):
        return base
    privado = get_nicho_bof_cine_redis().get_json(
