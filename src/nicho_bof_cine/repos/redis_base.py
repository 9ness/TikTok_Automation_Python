"""Wrapper Upstash REST especializado para Nicho BOF Cinematográfico (`nicho_bof_cine:`).
...
from src.nicho_bof_cine.config import REDIS_PREFIX


class NichoBofCineRedis:
    """Cliente Redis con prefijo `nicho_bof_cine:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoBofCineRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoBofCineRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: NichoBofCineRedis | None = None


def get_nicho_bof_cine_redis() -> NichoBofCineRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = NichoBofCineRedis()
    return _INSTANCE
