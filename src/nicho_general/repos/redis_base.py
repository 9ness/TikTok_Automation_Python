"""Wrapper Upstash REST especializado para Nicho General · UGC (`nicho_general:`).
...
from src.nicho_general.config import redis_prefix


class NichoGeneralRedis:
    """Cliente Redis con prefijo `nicho_general:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoGeneralRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoGeneralRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: NichoGeneralRedis | None = None


def get_nicho_general_redis() -> NichoGeneralRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = NichoGeneralRedis()
    return _INSTANCE
