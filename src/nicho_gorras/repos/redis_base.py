"""Wrapper Upstash REST especializado para Nicho Gorras (`nicho_gorras:`).
...
from src.nicho_gorras.config import REDIS_PREFIX


class NichoGorrasRedis:
    """Cliente Redis con prefijo `nicho_gorras:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoGorrasRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoGorrasRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: NichoGorrasRedis | None = None


def get_nicho_gorras_redis() -> NichoGorrasRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = NichoGorrasRedis()
    return _INSTANCE
