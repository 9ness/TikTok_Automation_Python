"""Wrapper Upstash REST especializado para Nicho Ropa Sin Personas (`nicho_ropa:`).
...
from src.nicho_ropa.config import redis_prefix


class NichoRopaRedis:
    """Cliente Redis con prefijo `nicho_ropa:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoRopaRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoRopaRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: NichoRopaRedis | None = None


def get_nicho_ropa_redis() -> NichoRopaRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = NichoRopaRedis()
    return _INSTANCE
