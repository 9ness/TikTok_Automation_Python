"""Wrapper Upstash REST especializado para Nicho Ropa Con Personas (`nicho_ropa_personas:`).
...
from src.nicho_ropa_personas.config import REDIS_PREFIX


class NichoRopaPersonasRedis:
    """Cliente Redis con prefijo `nicho_ropa_personas:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoRopaPersonasRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[NichoRopaPersonasRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: NichoRopaPersonasRedis | None = None


def get_nicho_ropa_personas_redis() -> NichoRopaPersonasRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = NichoRopaPersonasRedis()
    return _INSTANCE
