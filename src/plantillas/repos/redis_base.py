"""Wrapper Upstash REST especializado para las plantillas (`plantillas:`).
...
from src.plantillas.config import redis_prefix


class PlantillasRedis:
    """Cliente Redis con prefijo `plantillas:` por defecto.
    ...
    def _get(self, path: str, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[PlantillasRedis] GET {path} error: {e}")
            return None
    ...
    def _post(self, path: str, body: bytes | None = None, *, timeout: float = 10) -> Any:
        ...
        try:
            ...
            print(f"[PlantillasRedis] POST {path} error: {e}")
            return None
...
# Singleton perezoso
_INSTANCE: PlantillasRedis | None = None


def get_plantillas_redis() -> PlantillasRedis:
    global _INSTANCE
    if _INSTANCE is None:
        _INSTANCE = PlantillasRedis()
    return _INSTANCE
